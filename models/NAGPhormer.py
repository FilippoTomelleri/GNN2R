...
class AdaptedNAGphormer(GeneralSuperclass):

    def __init__(self, rel2embeds: dict, in_channes, out_channels):
        super(AdaptedNAGphormer, self).__init__()
        self.hid_dim = out_channels

        self.rel_embeds = [rel2embeds[r_id].cuda() for r_id in range(len(rel2embeds))]

        self.__model = TransformerModel(hops=2, n_class=2, input_dim=in_channes, pe_dim=768)

        # initial embedding of non-topic entities
        self.non_top_embed = nn.Parameter(torch.FloatTensor(1, self.hid_dim))
    ...
    def forward(self, que_embeds: FloatTensor, r: FloatTensor, num_subg_ents: int,
        ...
        initail_embeds = self.non_top_embed.repeat(num_subg_ents, 1)
        return (self.__model(initail_embeds), # size: (num_ents_in_subg, hid_dim) ???
                que_embeds)
